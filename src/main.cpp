...
/* ---------- KONFIGURASI PIN ---------- */
// Row PA0-PA7, Col PB0,PB1,PB2,PB10-PB14 (Sesuai kode awal Anda)
...
void loop() {
  // Wajib dipanggil agar koneksi USB tetap aktif
  usbMIDI.poll();
  ...
  for (int c = 0; c < 8; c++) {
    ...
    for (int r = 0; r < 8; r++) {
      ...
      if (pressed != matrixState[r][c]) {
        ...
        uint8_t note = 48 + (r * 8 + c); // C3 sebagai nada dasar
        
        if (pressed) {
          usbMIDI.sendNoteOn(0, note, 100); // Channel 1, Note, Vel 100
        } else {
